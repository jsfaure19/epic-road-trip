...
const ERTStepper = (props) => {
  const [isNewWaypointFormVisible, setIsNewWaypointFormVisible] = React.useState(false)
      ...
      return (
        <>
         ...
         <Autocomplete
												...
												onChange={(e)=> {props.addWaypoint(searchResult[e.target.value]); setIsNewWaypointFormVisible(false)}}
												sx={{ width: 300 }}
												renderInput={(params) => <TextField {...params} label="Waypoint" onChange={(e)=>setSearchValue(e.target.value)} value={searchValue}/>}
												/>
                        :<IconButton onClick={()=>setIsNewWaypointFormVisible(true)}><AddIcon/></IconButton> }
          <Grid item xs={12} container sx={{ justifyContent: "space-between" }}>
